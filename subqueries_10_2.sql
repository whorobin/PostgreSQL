...
-- valu insert 
INSERT INTO employeees (employee_name, department_name, salary, hire_date) VALUES 
...
-- Question : -- Retrieve all employees whose salary is greater than the highest salary of the HR department

-- Answer (1: table ta check korlam, 2:'HR' department a highest salary ber korbo, 3: 'HR' er jei highest salary ache tar theke beshi salary er list ta ber korbo (ekhane ekta querie er moddhe ro eta querie lekha hobe R etai sub querie)  )

    -- SELECT * FROM employeees;
    -- SELECT max(salary) FROM employeees WHERE department_name = 'HR';
    SELECT * from employeees WHERE salary > (SELECT max(salary) from employeees WHERE department_name = 'HR')
